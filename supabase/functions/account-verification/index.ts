...
async function hmacHex(message: string) {
  if (typeof globalThis?.crypto?.subtle?.importKey === "function") {
    const key = await globalThis.crypto.subtle.importKey("raw", new TextEncoder().encode(VERIFICATION_HMAC_SECRET), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
...
async function insertHashedCode(userId: string, method: string, destination: string) {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/account_verification_codes`, { method: "POST", headers: { Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`, apikey: SUPABASE_SERVICE_ROLE_KEY, "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify(payload) });
  if (!res.ok) throw new Error(`insertHashedCode failed: ${res.status} ${await res.text()}`);
...
async function sendWithSendGrid(destination: string, subject: string, text: string) {
  ...
  const payload = { personalizations: [{ to: [{ email: destination }] }], from: { email: "no-reply@trinket.com" }, subject, content: [{ type: "text/plain", value: text }] };
...
async function sendWithTwilio(destination: string, bodyText: string) {
  ...
  const params = new URLSearchParams(); params.append("To", destination); params.append("From", TWILIO_FROM); params.append("Body", bodyText);
...
async function findLatestRow(userId: string) {
  ...
  if (!res.ok) return null; const data = await res.json(); return data && data[0] ? data[0] : null;
}
...
serve(async (req: Request) => {
  try {
    ...
    if (req.method === "POST" && url.pathname.endsWith("/send")) {
      ...
      if (!userId || !method || !destination) return new Response(JSON.stringify({ ok: false, error: "missing" }), { status: 400 });
    ...
    if (req.method === "POST" && url.pathname.endsWith("/verify")) {
      ...
      if (!userId || !code) return new Response(JSON.stringify({ ok: false, error: "missing" }), { status: 400 });
      ...
      if (hashed !== row.hashed_code) {
        await fetch(`${SUPABASE_URL}/rest/v1/account_verification_codes?id=eq.${row.id}`, { method: "PATCH", headers: { Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`, apikey: SUPABASE_SERVICE_ROLE_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ attempts: (row.attempts || 0) + 1 }) });
        return new Response(JSON.stringify({ ok: false, reason: "bad" }), { status: 400 });
      }
      await fetch(`${SUPABASE_URL}/rest/v1/account_verification_codes?id=eq.${row.id}`, { method: "PATCH", headers: { Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`, apikey: SUPABASE_SERVICE_ROLE_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ used: true }) });
...
// Single-file account-verification Edge Function
import { serve } from "https://deno.land/std@0.177.0/http/server.ts";
...
serve(async (req: Request) => {
  try {
    ...
    if (req.method === "POST" && url.pathname.endsWith("/send")) {
      const body = await req.json();
      // Accept both userId (camelCase) and user_id (snake_case) to be resilient to different clients
      const userId = body.userId ?? body.user_id ?? '';
      const method = body.method;
      // Accept destination from multiple possible fields
      const destination = body.destination ?? body.email ?? body.phone ?? '';
      if (!userId || !method || !destination) return new Response(JSON.stringify({ ok: false, error: "missing" }), { status: 400 });
